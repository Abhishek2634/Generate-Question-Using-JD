...
import {
  Table, Input, Card, Button, Space, Tag, Typography, Modal, Divider, Row,
  Col, Statistic, Tooltip, Popconfirm, Badge, Select, message
} from "antd";
import {
  SearchOutlined, EyeOutlined, DeleteOutlined, ReloadOutlined,
  CalendarOutlined, TrophyOutlined, UserOutlined, ClockCircleOutlined,
...
export default function InterviewerDashboard() {
  ...
  const exportResults = () => {
    setLoading(true);
    try {
      const csvHeaders = ["Name", "Email", "Phone", "Score", "Performance", "Completed At", "Duration", "Summary"];
      const csvData = filteredCandidates.map((candidate) => {
        ...
        return [
          `"${candidate.name}"`, `"${candidate.email}"`, `"${candidate.phone}"`, candidate.score,
          performance, `"${formatDate(candidate.completedAt)}"`,
          `"${formatDuration(candidate.duration)}"`, `"${candidate.summary.replace(/"/g, '""')}"`
        ].join(",");
      });
      ...
      render: (score: number) => (
        <div style={{ textAlign: "center" }}>
          <div style={{
            ...
            color: getScoreColor(score), marginBottom: 4
          }}>{score}/100</div>
          {getScoreTag(score)}
        </div>
      ),
      ...
      render: (completedAt: string, record: CandidateWithKey) => (
        <div>
          <div style={{ display: "flex", alignItems: "center", marginBottom: 4 }}>
            ...
            <Text style={{ fontSize: "12px" }}>{formatDate(completedAt)}</Text>
          </div>
          <div style={{ display: "flex", alignItems: "center" }}>
            <ClockCircleOutlined style={{ marginRight: 4, color: "#eb2f96" }} />
            <Text style={{ fontSize: "12px" }}>{formatDuration(record.duration)}</Text>
          </div>
        </div>
      ),
      width: 150,
  ...
  render: (_: unknown, record: CandidateWithKey) => (
    <Space>
      {/* Resume button - shows only if resumeUrl exists */}
      {record.resumeUrl && (
              ...
              {selectedCandidate.answers.map((item, index) => (
                <Card key={index} size="small" style={{ marginBottom: 16 }}>
                  ...
                  <div style={{
                    ...
                    borderRadius: "6px", marginBottom: 12, border: "1px solid #d9d9d9"
                  }}>
